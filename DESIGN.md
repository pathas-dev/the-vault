...
- **Display & Headlines (Noto Serif KR):** These are your "Editorial Statements." Use `display-lg` for high-impact screens. The serif evokes the history and "weight" of the _Master Ledger_.
...
A specialized list item for _획득물 기록_ (Secure Phase Results). It uses `notoSerif` for the value and `manrope` (label-sm) for the metadata, separated by a wide `spacing-8` gap to emphasize the "heist" data aspect.
...
- **Intentional Asymmetry:** In the _작전 시작_ (Briefing) screens, align text to the left at `10%` screen width, but place supporting imagery or data at `60%` width to create a non-linear, sophisticated ocular path.```
````
