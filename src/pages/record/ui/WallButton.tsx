  ...
  return (
    <div className={`relative h-0 z-10 ${className || ''}`}>
      <button
        ...
        className="absolute inset-x-0 -top-2 h-4 md:-top-2.5 md:h-5 flex items-center justify-center cursor-pointer group"
        title={`가로벽 ${value}`}
      >
        <div
          className={`h-4 md:h-5 flex-1 rounded-sm border transition-all duration-300 mx-0.5 md:mx-1 ${
            isSelected
        ...
        >
          <span
            className={`serif-text text-label-sm font-black select-none transition-all duration-300 hidden ${
              isSelected
                ? 'text-tertiary'
  ...
  return (
    <div className={`relative w-0 z-10 ${className || ''}`}>
      <button
        ...
        className="absolute inset-y-0 -left-2 w-4 md:-left-2.5 md:w-5 flex flex-col items-center justify-center cursor-pointer group"
        title={`세로벽 ${value}`}
      >
        <div
          className={`w-4 md:w-5 flex-1 rounded-sm border transition-all duration-300 my-0.5 md:my-1 ${
            isSelected
              ? 'bg-tertiary/20 border-tertiary shadow-[0_0_12px_rgba(241,201,125,0.4)]'
              : 'bg-transparent border-dashed border-tertiary/30 group-hover:border-tertiary/50 group-hover:bg-tertiary/5'
          }`}
        >
          <span
            className={`text-label-sm font-black select-none transition-all duration-300 hidden ${
              isSelected
                ? 'text-tertiary'
                : 'text-tertiary/30 group-hover:text-tertiary/50'
            }`}
