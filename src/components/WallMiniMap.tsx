...
export function WallMiniMap({ horizontalWall, verticalWall, startPoint }: WallMiniMapProps) {
  ...
  return (
    <div className="flex items-stretch" style={{ width: '120px', height: '56px' }}>
      {/* 401 */}
      ...
      {/* 1xx column */}
      <div className="flex flex-col flex-1">
        <div className={`border border-l-0 border-outline-variant/30 bg-surface-container-lowest/30 flex-1 rounded-[1px] ${startPoint === 'A' ? 'border-r-error/60' : ''}`} />
        <div className="h-0 relative">
          <div className={`absolute left-0 right-0 -top-[1px] h-[3px] rounded-full ${horizontalWall === 'ㄷ' ? wallActive : wallInactive}`} />
        </div>
        <div className={`border border-l-0 border-t-0 border-outline-variant/30 bg-surface-container-lowest/30 flex-1 rounded-[1px] ${startPoint === 'B' ? 'border-r-error/60' : ''}`} />
      </div>

      {/* Entry point indicators */}
      {startPoint && (
        <div className="flex flex-col self-stretch w-2 ml-px">
          <div className={`flex-1 flex items-center ${startPoint === 'A' ? '' : 'opacity-0'}`}>
            <div className={`w-[4px] h-3 rounded-full ${entryActive}`} />
          </div>
          <div className={`flex-1 flex items-center ${startPoint === 'B' ? '' : 'opacity-0'}`}>
            <div className={`w-[4px] h-3 rounded-full ${entryActive}`} />
          </div>
        </div>
      )}
